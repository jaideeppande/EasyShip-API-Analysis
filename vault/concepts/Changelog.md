...
* **(Feature)** **New fields `official_name` and `nickname` in courier service endpoint**\
  ...
  Read the documentation [[List all Courier Services]].
...
* **(Feature)** **New Endpoint for searching Canada Post post offices**\
  Learn more about Canada Post location endpoint [[List Canada Post Locations]]
...
* **(Feature)** **Improvements on Manifests Creation**\
  ...
  Learn more about creating manifest [[Create a Manifest]].
...
* **(Feature)** **Adding New Filter to List all Addresses**\
  ...
  Learn more about the changes [[List all Addresses]].
* **(Feature)** **Endpoint for Tracking Update in Sandbox Environment**\
  Users can now update tracking information through the Sandbox environment. This endpoint is specifically for testing tracking status updates and is exclusively available within Sandbox environment. Read the documentation [[Update a tracking]].
* **(Feature)** **`accepts`filter on List all Courier Services**\
  Now we can use `accepts` filter for listing all courier services. The documentation is available [[List all Courier Services]].
...
* New [[Scopes]] with support for `read` and `write` access.
* Enhanced [[Errors]] responses with links to our documentation and knowledge base.
...
* [Labels API](https://developers.easyship.com/v2023.01/reference/labels_create)  was deprecated in favor of Batch API ( [[Create a Batch of Labels]]).
